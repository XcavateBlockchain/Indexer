...
```
                    ...
                    └───────────────┬──────────────┘
                                    ...
                                    ▼
                    ┌─────────────────────────────┐
                    │  carbon 0.12.0 decoder       │   crates/whitelist-decoder
                    │  (generated by carbon-cli,   │   (never hand-edited — see
                    │   never hand-edited)         │    README "Regenerating")
                    └───────────────┬──────────────┘
        ...
        │                                                    │
        │  InstructionProcessor   AccountProcessor  AccountDeletionProcessor
        │  (mapping.rs: old       (account state,   (redundant close path,
        │   mappingHandlers.ts     slot-guarded)      ADR-14)
        │   ported to Rust)                                  │
        │         │                    │                     │
        │         └────────────┬───────┴─────────────────────┘
        │                      ▼
        ...
        └──────────────────────┬──────────────────────────────┘
                    ...
                    ┌─────────────────────────────┐
                    │          Postgres            │
                    │  program_instructions        │  append-only history (§2)
                    │  config / admin / role_account│  current-state (§2)
                    │  whitelist_actions            │  parity/audit log (§3)
                    │  admins_view / role_          │  derived, folded (§3)
                    │  assignments_view / config_view│
                    │  sync_state / backfill_cursor │  pipeline bookkeeping
                    └───────────────┬───────────────┘
...
- **`program_instructions`** — append-only, one row per decoded instruction (including
  nested/CPI instructions, collapsed to `(outer, inner)` index pairs), `ON CONFLICT DO
  NOTHING`. This is the raw history: every instruction the program ever executed, in full,
  with its JSONB-encoded arguments (`ADR-4`). Nothing downstream trusts insertion order —
  a reprocessed instruction is a no-op.
- **`config` / `admin` / `role_account`** — slot-guarded, current-only account state, one row
  per PDA (`ADR-2`, `ADR-6`). These hold *only* fields that exist on-chain, so they stay
  droppable and rebuildable from a fresh `getProgramAccounts` snapshot at any time
  (`ADR-3`) — a close is a soft `UPDATE`, never a `DELETE` (`ADR-7`).
...
| `getProgramAccounts` snapshot (`indexer snapshot`) | **current state** on an idle program | `config`/`admin`/`role_account`, `sync_state.snapshot_slot` |
...
`last_contiguous_slot + 1`, and only then advances the frontier to the pre-crawl tip. On a
quiet program this is one page (~1 RPC call) per cycle — about 576 requests/day, under 2%
of Alchemy's free-tier budget.
...
| The 3 instructions that close a PDA (`remove_admin`, `remove_role`, `renounce_role`) setting `active = false` on the entity | additionally emit `WriteOp::CloseAdmin` / `CloseRoleAccount` from the **instruction** processor (ADR-14), not just the deletion pipe |
...
```
crates/
  whitelist-decoder/   generated by carbon-cli from idls/xcavate_whitelist.json — never hand-edited (README §"Regenerating")
  indexer/              the Carbon pipeline binary: run / backfill / snapshot / smoke-grpc
  api/                  the GraphQL API binary (Axum + Juniper), port 3010
migrations/             sqlx migrations, applied in filename order (0001..0006)
idls/                   xcavate_whitelist.json (indexed) + marketplace/property/regions.json (not yet indexed, ADR-19)
