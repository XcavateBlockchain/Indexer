...
| `INIT_PROPERTY_ASSET_WEBHOOK_URL` | indexer | *(unset → disabled)* | Outbound webhook target for property-asset registration (ADR-28): when a new `PropertyAsset` PDA is created (marketplace `init_property_assets`), the indexer's background loop POSTs one JSON document here. Unset/empty = no delivery loop is spawned and no external call is made (the durable `webhook_events` rows are still recorded). See [RUNBOOK.md](RUNBOOK.md) "Property-asset registration webhooks". |
| `WEBHOOK_INTERVAL` | indexer | `5` (seconds) | Webhook delivery poll interval (ADR-28): how often the background loop drains the `webhook_events` work set. Per-event backoff hides retries until due, so the interval costs nothing while nothing is pending. |
