...
# `cargo chef cook` cannot reconstruct them from `recipe.json` alone (verified empirically:
# without this, `cook` fails with "failed to read `/app/crates/whitelist-decoder/Cargo.toml`
# ... No such file or directory") and needs the real files -- copy just those crates ahead of
# `cook` so the dependency layer still only rebuilds when they (or recipe.json) change, not on
